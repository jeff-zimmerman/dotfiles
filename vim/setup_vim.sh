...
done <<< "$( find $PWD/vim/.vim_runtime -name '.gitrepo' | sort -r | xargs realpath --relative-to=$PWD )" 
#| xargs -I% echo \"$( git subrepo config $( dirname % ) remote | grep -oP 'https://.*\.git' ) $( dirname % )\" )"
